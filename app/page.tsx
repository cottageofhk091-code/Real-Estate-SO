...
/** 分析成功回数の永続化キー（リロード後も保持） */
const ANALYSIS_COUNT_STORAGE_KEY = 'property_analysis_count';

function readAnalysisCount(): number {
  try {
    const raw = localStorage.getItem(ANALYSIS_COUNT_STORAGE_KEY);
...
function writeAnalysisCount(count: number): void {
  try {
...
export default function Home() {
  ...
  /** 分析成功回数（localStorage 同期・トラッキング用。基本分析の無料無制限とは独立） */
  ...
  useEffect(() => {
    setAnalysisCount(readAnalysisCount());
  ...
  const handleAuthEmailSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    if (authMode === 'signup') {
      if (!authAgeGroup) {
        setAuthError('年代を選択してください。');
        return;
      }
      if (!authRegion) {
        setAuthError('地域（都道府県）を選択してください。');
        return;
      }
      if (!authAgreed) {
              ...
              {activeModal === 'terms' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <p style={{ margin: 0, color: COLORS.textMuted }}>
                    「物件セカンドオピニオン AI Pro」（以下、「当サービス」）のご利用にあたり、以下の免責事項をよくお読みください。当サービスを利用された場合、本内容に同意したものとみなします。
                  </p>
                  {[
                    {
                      t: '1. サービスの目的と情報の正確性',
                      b: '当サービスはAIを活用して物件情報を整理・分析するものであり、宅地建物取引業に基づく媒介行為、正式な不動産鑑定、または投資の勧誘を行うものではありません。分析結果の完全性・正確性を保証するものではありません。',
                    },
                    {
                      t: '2. 投資・契約判断に関する責任',
                      b: 'スコアやリスク分析は参考情報です。実際の不動産購入・賃貸契約等の意思決定は必ずご自身の責任で行い、必要に応じて宅地建物取引業者・不動産鑑定士・弁護士等の専門家へご相談ください。当サービスの利用結果に基づいて生じた損害について、運営者は一切責任を負いません。',
                    },
                    {
                      t: '3. 入力データの取り扱い',
                      b: '入力されたテキストおよび画像は、AIによる解析処理および品質向上のために使用されます。個人を特定できる情報（氏名・電話番号・詳細な住所等）の入力はお控えください。',
                    },
                  ].map((item) => (
              ...
              {activeModal === 'agreement' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  {[
                    ...
                    {
                      t: '2. 有料プラン・購入および支払い',
                      b: `（1）本サービスの利用料金（単発Pro機能、月額Proプラン等）は、サイト上に表示される価格とします（単発Pro機能：税込${formatYen(PRICE_SINGLE_YEN)}等）。\n（2）お支払い方法は、Stripe Payments Japan合同会社が提供する決済システムを通じたクレジットカード決済等によるものとします。\n（3）単発Pro機能は購入後直ちに適用されます。デジタルコンテンツおよびデジタル役務の性質上、購入・決済完了後の返金・返品・キャンセルには理由を問わず一切応じられません。`,
                    },
                    {
                      t: '3. 月額プランの自動更新と解約',
                      b: '月額Proプランは、解約手続きが行われない限り自動的に更新され、月額料金が課金されます。解約はマイページよりいつでも手続きが可能です。解約手続きを行った場合でも、すでに支払われた利用料金の返金（日割り計算含む）は行われません。',
                    },
                    {
                      t: '4. 禁止事項',
              ...
              {activeModal === 'tokushoho' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  {[
                    ...
                    {
                      t: '■ 支払時期',
                      b: '・単発Pro機能：ご注文時（即時決済）\n・月額Proプラン：初回購入時、および翌月以降の自動更新日に即時決済',
                    },
                    {
                      t: '■ 役務の提供時期',
                      b: '決済完了後、直ちに対象のPro機能・分析機能をご利用いただけます。単発Pro機能は購入後即時に適用されます。',
                    },
                    {
                      t: '■ 返品・返金・キャンセルについて',
                      b: `・単発Pro機能（税込${formatYen(PRICE_SINGLE_YEN)}）：購入後直ちに対象機能が適用されます。デジタル役務の性質上、決済完了後の返金・キャンセルには応じられません。\n・月額Proプラン：デジタル役務の性質上、決済完了後の返金・返品、および途中解約による日割り返金には原則として応じられません。解約は次回更新日までにマイページより手続きを行っていただくことで、次回以降の課金を停止できます。`,
                    },
                  ].map((item) => (
                    <div key={item.t} style={{ backgroundColor: COLORS.cardAlt, padding: '16px', borderRadius: '12px', border: `1px solid ${COLORS.border}` }}>
                      <h4 style={{ color: COLORS.accentStrong, margin: '0 0 6px 0', fontSize: '14px' }}>{item.t}</h4>
